...
const REAL_BUS_LINES = [
  {
    corridor: 'tlv_north_center',
    lineNum: '5',
    operator: 'דן',
    title: 'קו 5 (דן): רכבת תל אביב סבידור מרכז ⟵ דיזנגוף / סוקולוב',
    originTerminal: 'מסוף 2000 / סבידור מרכז',
    destTerminal: 'תל אביב חוף מציצים',
    departureSchedule: [0, 8, 16, 24, 32, 40, 48, 56],
    intervalMins: 8,
    boardStation: 'דרך נמיר / ז\'בוטינסקי',
    stopId: '21102',
    alightStation: 'סוקולוב / בזל',
    alightStopId: '21250',
    alts: ['קו 25 (דן)', 'קו 125 (דן)', 'קו 66 (דן)']
  },
  {
...
function resolveUrbanOrRegionalBus(origin, dest) {
  if (origin.lat >= 32.02 && origin.lat <= 32.14 && origin.lon >= 34.74 && origin.lon <= 34.85) {
...
function runMasterTransitEngine(body) {
  ...
  if (totalDistanceKm <= 7.0) {
    ...
    return [{
      ...
      whyReason: `נסיעה עירונית ישירה ב${bus.title}. עולים ב-${bus.boardStation} ויורדים ב-${bus.alightStation}.`,
      timeline: [
        {
        ...
        },
        {
          ...
          alternatives: bus.alts.map(a => ({ name: a, desc: 'קו מקביל' })),
  ...
  timeline.push({
    ...
    alternatives: [
      { name: `${trainLeg.line.name} (רכבת עוקבת)`, desc: `${trainLeg.directionName} • אותו רציף` }
    ],
  ...
  if (hasDestPickup) {
    ...
    timeline.push({
      ...
      alternatives: connectingBus.alts.map(a => ({ name: a, desc: 'קו חלופי' })),
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif; -webkit-tap-highlight-color: transparent; }
    ...
    .btn-copy-origin { background: var(--tint); border: 1px solid rgba(0,122,255,0.2); border-radius: 8px; font-size: 12px; font-weight: 600; color: var(--blue); padding: 4px 8px; cursor: pointer; margin-right: 4px; }
    ...
    .alt-item-btn { background: var(--bg); padding: 10px 12px; border-radius: 10px; margin-bottom: 8px; font-size: 13px; border: 1px solid var(--border); text-align: right; width: 100%; display: block; }
...
<body>
  ...
  <div id="alt-modal" class="modal-backdrop">
    <div class="modal-box">
      ...
      <div style="font-size:12.5px; color:var(--sec); margin-bottom:8px;">זמני יציאה קרובים (לפי לו"ז רשמי):</div>
      <div id="alt-departures-list" style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:14px;"></div>
      <div style="font-size:12.5px; color:var(--sec); margin-bottom:8px;">קווים חלופיים:</div>
  ...
  <div class="card" style="overflow:visible;">
    ...
    <div class="input-row">
      ...
      <label>יעד</label>
      <input type="text" id="userDestInput" placeholder="לאן להגיע? (למשל: סוקולוב תל אביב, בזל)" autocomplete="off" oninput="handleInputAutocomplete('userDest')">
  ...
  <div id="driver-route-section">
    ...
    <div class="card" style="overflow:visible;">
      <div class="input-row">
        ...
        <button type="button" class="btn-copy-origin" title="העתק ממוצא שלי" onclick="copyUserOriginToDriver()">👤=🚗 שכפל מוצא</button>
  ...
  <script>
    ...
    // מיקום נוכחי עמיד עם קריאת מטמון קודמת (למניעת Timeout ב-iOS)
    function useCurrentGPS() {
      ...
      // קריאה ראשונה עם בדיקת מטמון (Cache Hit) של 5 דקות - פועל מידית באייפון
      navigator.geolocation.getCurrentPosition(
        ...
        err => {
          // ניסיון fallback עם דיוק רגיל (Cellular/WiFi)
          navigator.geolocation.getCurrentPosition(
            pos2 => {
            ...
            () => {
              status.innerText = 'המיקום לא אותר. ניתן להקליד כתובת או לסמן ישירות במפה 🗺️';
              btn.innerText = '📍';
            },
    ...
    function openAlternativesModal(lineName, rawMins, schedMinutesJson, altsJson) {
      ...
      if (alternatives.length === 0) {
        ...
        alternatives.forEach(alt => {
          ...
          div.className = 'alt-item-btn';
          div.innerHTML = '<b>' + alt.name + '</b><div style="font-size:12px; color:var(--sec); margin-top:2px;">' + (alt.desc || 'מסלול חלופי') + '</div>';
      ...
      if (!exactCoords.userOrigin || exactCoords.userOrigin.name !== uOriginVal) {
        ...
        if (direct) {
          ...
          if (list && list.length > 0) {
            ...
            alert('לא נמצא מיקום עבור: ' + uOriginVal);
            return;
          }
        }
      }
      ...
      if (!exactCoords.userDest || exactCoords.userDest.name !== uDestVal) {
        ...
        if (direct) {
          ...
          if (list && list.length > 0) {
            ...
            spinner.style.display = 'none';
            btnText.innerText = 'מצא לי דרך';
            alert('לא נמצא מיקום עבור: ' + uDestVal);
            return;
          }
        }
      }
    ...
    function renderResults(results) {
      ...
      if (!results || results.length === 0) {
        container.innerHTML = '<div class="card">לא נמצאו נקודות מעבר מתאימות. נסה לשנות את טווחי הזמנים.</div>';
        return;
      }
