...
async function geocodeAddress(text) {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/7.0' } });
...
const ALL_HUBS = [
  { id: 'h_hamovill', name: 'צומת המוביל / מחלף המוביל', type: 'bus', transitType: 'אוטובוס מהיר', lineName: 'קו 430 (סופרבוס)', platform: 'רציף 2', dropStation: 'מרכזית טבריה / עמקים', intermediateStops: 4, lat: 32.7562, lon: 35.2341 },
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif; -webkit-tap-highlight-color: transparent; }
    ...
    /* Map Picker Modal - מותאם למובייל */
...
<body>
  ...
  <!-- Modal Map Picker עם כפתור חיפוש ייעודי והקפצה מיידית -->
  <div id="map-modal" class="map-modal">
    ...
    <div class="map-modal-search">
      <input type="text" id="modalSearchInput" placeholder="הקלד שם מקום, עיר או צומת..." autocomplete="off" oninput="handleModalSearch()" onkeydown="if(event.key==='Enter'){event.preventDefault();executeModalSearch();}">
