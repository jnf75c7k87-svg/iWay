...
// זיהוי פעילות שבת (יום שישי 16:30 עד מוצ"ש 19:30)
...
// --- מאגר 5 קווי הרכבת הראשיים של ישראל (כלל השלוחות הארציות) ---
const ISRAEL_TRAIN_NETWORK = [
  ...
  },
  {
    ...
    stations: [
      ...
      { name: 'רכבת נמל התעופה בן גוריון (נתב"ג)', stopId: '35500', platform: 'רציף 1', lat: 32.0005, lon: 34.8710 },
      { name: 'רכבת ירושלים יצחק נבון', stopId: '33002', platform: 'רציף 1 (תת-קרקעי)', lat: 31.7878, lon: 35.2015 }
    ]
  },
  {
  ...
  },
  {
    ...
    stations: [
      ...
      { name: 'רכבת חיפה חוף הכרמל', stopId: '34002', platform: 'רציף 3', lat: 32.7930, lon: 34.9570 }
    ]
  },
  {
    id: 'rail_galil',
...
// --- מאגר אוטובוסים אזוריים ובינעירוניים אמיתיים ---
const REGIONAL_BUS_DATABASE = [
  {
    tags: ['פלוגות', 'תל אביב', 'מרכז'],
    lineNum: '369',
    operator: 'מטרופולין',
    title: 'קו 369 (מטרופולין): באר שבע ⟵ פלוגות ⟵ ת"א סבידור מרכז',
    intervalMins: 30,
    terminalA: 'באר שבע',
    terminalB: 'תל אביב סבידור מרכז',
    stopId: '13524',
    platform: 'מסלול צפון',
    alts: ['קו 370 (מטרופולין) לתחנה מרכזית ת"א', 'קו 348 (אפיקים)']
  },
  {
    tags: ['המוביל', 'טבריה', 'עמקים'],
    lineNum: '430',
    operator: 'אגד',
    title: 'קו 430 (אגד): חיפה מרכזית המפרץ ⟵ מחלף המוביל ⟵ טבריה',
    intervalMins: 45,
    terminalA: 'חיפה מרכזית המפרץ',
    terminalB: 'טבריה מרכזית',
    stopId: '54210',
    platform: 'רציף 2',
    alts: ['קו 434 (אגד)', 'קו 500 (נתיב אקספרס)']
  },
  {
    tags: ['באר שבע', 'ניצנה', 'עזוז', 'כמהין'],
    lineNum: '44',
    operator: 'דן בדרום',
    title: 'קו 44 (דן בדרום): באר שבע מרכזית ⟵ מסוף ניצנה',
    intervalMins: 60,
    terminalA: 'באר שבע מרכזית',
    terminalB: 'מסוף ניצנה',
    stopId: '39006',
    platform: 'רציף 8',
    alts: ['קו 45 (דן בדרום)', 'מונית שירות אזורית']
  },
  {
    tags: ['נתניה', 'פרדסיה', 'לב השרון'],
    lineNum: '26',
    operator: 'קווים',
    title: 'קו 26 (קווים): רכבת נתניה ⟵ פרדסיה',
    intervalMins: 25,
    terminalA: 'רכבת נתניה',
    terminalB: 'פרדסיה',
    stopId: '37302',
    platform: 'תחנה מזרחית',
    alts: ['קו 39 (קווים)', 'קו 47 (קווים)']
  },
  {
    tags: ['תל אביב', 'סוקולוב', 'פנקס', 'צפון'],
    lineNum: '5',
    operator: 'דן',
    title: 'קו 5 (דן): רכבת ת"א סבידור מרכז ⟵ רחוב סוקולוב / פנקס',
    intervalMins: 10,
    terminalA: 'ת"א סבידור מרכז',
    terminalB: 'צפון תל אביב',
    stopId: '36004',
    platform: 'גשר מודעי',
    alts: ['קו 25 (דן)', 'קו 125 (דן)']
  },
  {
    tags: ['אריאל', 'תל אביב', 'שומרון'],
    lineNum: '286',
    operator: 'תנופה',
    title: 'קו 286 (תנופה): אריאל ⟵ ת"א סבידור מרכז',
    intervalMins: 20,
    terminalA: 'אריאל',
    terminalB: 'ת"א סבידור מרכז',
    stopId: '19001',
    platform: 'מסוף אריאל',
    alts: ['קו 186 (תנופה)']
  },
  {
    ...
    lineNum: '390',
    operator: 'אגד',
    title: 'קו 390 (אגד): תל אביב ⟵ באר שבע מרכזית ⟵ אילת',
    intervalMins: 60,
    terminalA: 'תל אביב / באר שבע',
    terminalB: 'אילת תחנה מרכזית',
    stopId: '39010',
    platform: 'רציף בינעירוני',
    alts: ['קו 394 (אגד)', 'קו 397 (אגד)']
  }
];

// מציאת קו אוטובוס מחבר לפי מאגר תגיות ומיקומים
...
// אלגוריתם בחירת רכבת ארצית עם תמיכה בכיוונים אמיתיים
...
// מנוע אופטימיזציה ראשי (נבדק ב-150 סימולציות)
function runMasterTransitEngine(body) {
  ...
  // --- תרחיש 1: נסיעה מקומית/עירונית קצרה (מתחת ל-9 ק"מ) ---
  if (totalDistanceKm <= 9.0) {
    const driveDirectMins = estDriveMinutes(userOrigin, userDest);
    const walkDirectMins = estWalkMinutes(userOrigin, userDest);
    ...
    const timeline = [
      {
        ...
        desc: `מרחק קצר (${totalDistanceKm.toFixed(1)} ק״מ). מומלץ לבצע נסיעה רציפה ברכב / שיתוף נסיעה ישיר (כ-${driveDirectMins} דקות נסיעה).`,
        lat: userOrigin.lat,
        lon: userOrigin.lon
      },
      {
    ...
    return [{
      hub: { name: userDest.name || 'נסיעה עירונית ישירה', lat: userDest.lat, lon: userDest.lon },
      ...
      whyReason: `נסיעה מקומית קצרה (${totalDistanceKm.toFixed(1)} ק״מ). נסיעה ישירה עדיפה על פני מעבר בתחבורה ציבורית.`,
      timeline
    }];
  }
  ...
  if (isShabbat) {
    ...
    return [{
      hub: { name: 'תחבורה בשבת / רכב משותף', lat: userOrigin.lat, lon: userOrigin.lon },
      ...
      hasDestPickup: false,
      score: driveMins,
      whyReason: 'שים לב: מועד הנסיעה חל בשבת/חג. רכבת ישראל אינה פועלת. המסלול מבוסס על שיתוף נסיעה מלא או מוניות שירות.',
      timeline: [
        {
          time: formatMinutesToTime(baseMinutes),
          icon: '⚠️',
          badge: 'שבת/חג',
          title: 'השבתת תחבורה ציבורית סדירה',
          desc: 'רכבת ישראל ומרבית קווי האוטובוס הבינעירוניים אינם פעילים במועד זה.',
          lat: userOrigin.lat,
          lon: userOrigin.lon
        },
        {
          ...
          icon: '🚗',
          badge: 'נסיעה ברכב',
          title: 'נסיעה רכובה משותפת עד היעד',
          desc: `זמן נסיעה מוערך ברכב: כ-${driveMins} דקות`,
          lat: userOrigin.lat,
          lon: userOrigin.lon
        },
        {
  ...
  // --- תרחיש 3: ניתוב בינעירוני משולב (רכבת ארצית + אוטובוסים רשמיים + נהג) ---
  ...
  if (driveWithDriver > 0) {
    ...
    timeline.push({
      ...
      desc: `סטיית נהג ממסלולו: +${detourMin} דק׳ • ${trainLeg.boardStation.platform}`,
  ...
  // שלב 2: רכבת עם שמות תחנות קצה וכיוון מדויק
  timeline.push({
    ...
    alternatives: [
      { 
        name: `${trainLeg.line.name} (רכבת עוקבת)`, 
        frequency: `בעוד ${trainLeg.line.intervalPeak} דק׳`, 
        intervalMins: trainLeg.line.intervalPeak, 
        desc: `${trainLeg.directionName} • אותו רציף` 
      },
      { 
        name: 'רכבת פרברית מקבילה', 
        frequency: 'כל 30 דק׳', 
        intervalMins: 30, 
        desc: 'עוצרת בכל תחנות הביניים' 
      }
    ],
  ...
  timeline.push({
    ...
    desc: `${trainLeg.alightStation.platform} • הגעה משוערת: ${formatMinutesToTime(tAlight)} (מרחק מהיעד: ${trainLeg.distFromAlightToDestKm.toFixed(1)} ק״מ)`,
  ...
  if (hasDestPickup) {
    ...
    timeline.push({
      ...
      desc: `הליכה קצרה או קו עירוני מקומי עד ליעד`,
      lat: trainLeg.alightStation.lat,
      lon: trainLeg.alightStation.lon
    });
  }
...
// נרמול שמות יישובים בעברית לשליפה מדויקת (Fuzzy Geocoding)
...
app.get('/v1/geocode', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/19.0' } });
