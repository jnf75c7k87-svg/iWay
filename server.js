...
const TRAIN_NETWORK_STATIONS = [
  { 
    ...
    lon: 34.7972,
    alternatives: [
      { name: 'רכבת 142', frequency: 'כל 30 דק׳', desc: 'רכבת מהירה לתל אביב וצפון' },
      { name: 'קו 380', frequency: 'כל 20 דק׳', desc: 'אוטובוס ישיר לתל אביב' }
    ]
  },
  { 
    name: 'רכבת נתניה', 
    ...
    lon: 34.8660,
    alternatives: [
      { name: 'רכבת 144', frequency: 'כל 30 דק׳', desc: 'רכבת פרברית' },
      { name: 'קו 605', frequency: 'כל 15 דק׳', desc: 'אוטובוס ישיר לתל אביב' }
    ]
  },
  { 
    name: 'רכבת תל אביב סבידור מרכז', 
    stopId: '36004', 
    platform: 'רציף 4', 
    trainNum: 'רכבת 218', 
    lat: 32.0835, 
    lon: 34.7983,
    alternatives: [
      { name: 'דנקל קו אדום R1', frequency: 'כל 6 דק׳', desc: 'רכבת קלה ת"א-בת ים-פ"ת' },
      { name: 'רכבת 142', frequency: 'כל 20 דק׳', desc: 'רכבת חוף לחיפה ונהריה' }
    ]
  },
  { 
    name: 'רכבת תל אביב השלום', 
    stopId: '36006', 
    platform: 'רציף 3', 
    trainNum: 'רכבת 142', 
    lat: 32.0734, 
    lon: 34.7925,
    alternatives: [
      { name: 'רכבת 218', frequency: 'כל 20 דק׳', desc: 'רכבת מהירה לצפון' },
      { name: 'קו 501', frequency: 'כל 12 דק׳', desc: 'אוטובוס ישיר להרצליה ושרון' }
    ]
  },
  { 
    name: 'רכבת קריית גת', 
    ...
    lon: 34.7738,
    alternatives: [
      { name: 'קו 369', frequency: 'כל 25 דק׳', desc: 'אוטובוס בינעירוני למרכז' }
    ]
  }
];
...
// קביעת קו תחבורה ספציפי ליעד הסופי במקום ניסוח גנרי
function resolveConnectingTransitLine(station, userDest) {
  const destName = (userDest.name || '').toLowerCase();
  
  if (station.name.includes('באר שבע')) {
    if (destName.includes('ניצנה') || destName.includes('כמהין') || destName.includes('עזוז')) {
      return {
        badge: 'קו 44',
        name: 'קו 44 (דן בדרום) למסוף ניצנה',
        stopId: '39006',
        platform: 'רציף 8',
        desc: 'יוצא מהתחנה המרכזית ב"ש הצמודה לרכבת (מעבר רגלי 2 דק׳)',
        intervalMins: 45,
        alternatives: ['קו 45 (דן בדרום)', 'קו 142 (מועצה אזורית)']
      };
    }
    return {
      badge: 'קו 60',
      name: 'קו 60 / 65 (מטרופולין) לכיוון היעד',
      stopId: '39010',
      platform: 'רציף 5',
      desc: 'נסיעה ישירה מהתחנה המרכזית',
      intervalMins: 30,
      alternatives: ['קו 64', 'קו 160']
    };
  }

  if (station.name.includes('נתניה')) {
    if (destName.includes('פרדסיה')) {
      return {
        badge: 'קו 26',
        name: 'קו 26 (קווים) לפרדסיה',
        stopId: '37302',
        platform: 'תחנת רכבת מזרח',
        desc: 'קו ישיר מצומת הרכבת עד למרכז פרדסיה',
        intervalMins: 20,
        alternatives: ['קו 39 (קווים)', 'קו 47 (קווים)']
      };
    }
    return {
      badge: 'קו 14',
      name: 'קו 14 (אקסטרה) לכיוון היעד',
      stopId: '37305',
      platform: 'רציף 2',
      desc: 'נסיעה עירונית ישירה',
      intervalMins: 15,
      alternatives: ['קו 25', 'קו 70']
    };
  }

  if (station.name.includes('תל אביב')) {
    if (destName.includes('סוקולוב') || destName.includes('צפון')) {
      return {
        badge: 'קו 5',
        name: 'קו 5 (דן) לרחוב סוקולוב / פנקס',
        stopId: '21102',
        platform: 'גשר מודעי',
        desc: 'עובר דרך נמיר וארלוזרוב ומוריד בסמוך לרחוב',
        intervalMins: 8,
        alternatives: ['קו 25 (דן)', 'קו 125 (דן)']
      };
    }
    return {
      badge: 'דנקל R1',
      name: 'הקו האדום של הדנקל / קו 1',
      stopId: '20101',
      platform: 'תחנה תת-קרקעית סבידור',
      desc: 'נסיעה מהירה לאורך ציר מרכזי',
      intervalMins: 6,
      alternatives: ['קו 40', 'קו 42']
    };
  }

  return {
    badge: 'קו 1',
    name: `קו 1 אזורי מ-${station.name} ליעד`,
    stopId: '10001',
    platform: 'מסוף מרכזי',
    desc: 'חיבור ישיר בתחבורה ציבורית',
    intervalMins: 20,
    alternatives: ['קו 2', 'קו 3']
  };
}

const ALL_HUBS = [
  { id: 'h_hamovill', name: 'מחלף המוביל (מסוף בינעירוני)', type: 'bus', transitType: 'אוטובוס מהיר', lineBadge: 'קו 430', lineName: 'קו 430 (סופרבוס) לצפון ולמרכז', stopId: '54210', platform: 'רציף 2', intervalMins: 20, alternatives: ['קו 380 (סופרבוס)', 'קו 500 (נתיב אקספרס)'], lat: 32.7562, lon: 35.2341 },
  { id: 'h_plugot', name: 'צומת פלוגות (תחנה בינעירונית)', type: 'bus', transitType: 'אוטובוס בינעירוני', lineBadge: 'קו 369', lineName: 'קו 369 (מטרופולין) לתל אביב', stopId: '13524', platform: 'מסלול צפון', intervalMins: 25, alternatives: ['קו 370 (מטרופולין)', 'קו 348 (אפיקים)'], lat: 31.6214, lon: 34.7478 },
  { id: 'h_kgat', name: 'רכבת קריית גת', type: 'train', transitType: 'רכבת ישראל', lineBadge: 'רכבת 142', lineName: 'רכבת מהירה למרכז (באר שבע ⟵ הרצליה)', stopId: '39002', platform: 'רציף 1', intervalMins: 30, alternatives: ['רכבת 218', 'קו 369'], lat: 31.6035, lon: 34.7738 },
  { id: 'h_tlv_hashalom', name: 'רכבת ת"א השלום (עזריאלי)', type: 'train', transitType: 'רכבת ישראל', lineBadge: 'רכבת 142', lineName: 'רכבת בינעירונית (באר שבע ⟵ נהריה)', stopId: '36006', platform: 'רציף 3', intervalMins: 20, alternatives: ['רכבת 144', 'קו 501'], lat: 32.0734, lon: 34.7925 },
  { id: 'h_tlv_savidor', name: 'ת"א סבידור מרכז', type: 'train', transitType: 'רכבת ישראל', lineBadge: 'רכבת 218', lineName: 'רכבת מהירה לצפון (ת"א ⟵ חיפה)', stopId: '36004', platform: 'רציף 4', intervalMins: 20, alternatives: ['דנקל R1', 'קו 40'], lat: 32.0835, lon: 34.7983 }
];
...
function runSmartOptimizer(body) {
  ...
  const { station: optimalAlightStation } = findClosestStationToDestination(userDest);
  const connectingTransit = resolveConnectingTransitLine(optimalAlightStation, userDest);
  ...
  for (const hub of ALL_HUBS) {
    ...
    if (routeType === 'driver_route') {
      ...
      if (rideFromOriginTime > 0) {
        ...
        const actualWalkMin = estWalkMinutes(userOrigin, hub);
        if (actualWalkMin <= maxWalkLimit) {
          isEligible = true;
          walkToHubMin = actualWalkMin;
          driveWithDriver = 0;
        }
      }
    }
    ...
    const trainRideMinutes = Math.max(8, Math.round((distBetweenStationsKm * 1.15) / 75 * 60));

    const driveFinalLeg = estDriveMinutes(optimalAlightStation, userDest);
    const hasDestPickup = destRideRadius > 0 && driveFinalLeg <= destRideRadius;
    const transitFinalMins = 18;
    const finalLegMinutes = hasDestPickup ? driveFinalLeg : transitFinalMins;

    const initialLegTime = driveWithDriver > 0 ? driveWithDriver : walkToHubMin;
    const waitTimeAtHub = 5;
    const totalTripMinutes = initialLegTime + trainRideMinutes + finalLegMinutes + waitTimeAtHub;
    ...
    const tBoardTransit = tDropoffAtHub + waitTimeAtHub;
    ...
    if (driveWithDriver > 0) {
      ...
      timeline.push({
        ...
        title: `הליכה רגלית לתחנת ${hub.name}`,
        desc: `מרחק הליכה מאושר לפי הסליידר (${walkToHubMin} מתוך ${maxWalkLimit} דק׳)`,
        lat: hub.lat,
        lon: hub.lon
      });
    }
    ...
    timeline.push({
      ...
      rawMinutes: tBoardTransit,
      intervalMins: hub.intervalMins || 20,
    ...
    timeline.push({
      ...
      desc: `${optimalAlightStation.platform} • שעת ירידה משוערת: ${formatMinutesToTime(tAlightTransit)}`,
    ...
    if (hasDestPickup) {
      timeline.push({
        time: formatMinutesToTime(tAlightTransit + 1),
        icon: '🚗',
        badge: 'איסוף ביעד',
        title: `איסוף ברכב מ-${optimalAlightStation.name}`,
        desc: `נסיעה ברכב כ-${driveFinalLeg} דקות ליעד (ברדיוס של עד ${destRideRadius} דק׳)`,
        lat: optimalAlightStation.lat,
        lon: optimalAlightStation.lon
      });
    } else {
      timeline.push({
        ...
        icon: '🚌',
        badge: connectingTransit.badge,
        isInteractiveLine: true,
        rawMinutes: tAlightTransit + 2,
        intervalMins: connectingTransit.intervalMins,
        alternatives: connectingTransit.alternatives,
        title: `עולים על ${connectingTransit.name}`,
        desc: `תחנה #${connectingTransit.stopId} (${connectingTransit.platform}) • ${connectingTransit.desc}`,
        lat: optimalAlightStation.lat,
        lon: optimalAlightStation.lon
      });
    }
    ...
    candidates.push({
      ...
      connectingLine: connectingTransit.badge,
      ...
      hasDestPickup,
      score,
      whyReason: `מעבר ב-${hub.name} (${hub.lineBadge}), רכבת ל-${optimalAlightStation.name} והמשך ישיר ב-${connectingTransit.badge}.`,
      timeline
    });
  }
...
app.get('/v1/geocode', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/16.0' } });
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif; -webkit-tap-highlight-color: transparent; }
    ...
    /* תגית קו אינטראקטיבית הניתנת ללחיצה */
    .step-tag-clickable { font-size: 11.5px; padding: 3px 8px; border-radius: 6px; font-weight: 700; background: #007AFF; color: #FFF; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; box-shadow: 0 2px 4px rgba(0,122,255,0.25); }
    .step-tag-clickable:active { transform: scale(0.96); }
    ...
    .modal-box { background: var(--card); border-radius: 20px; padding: 22px; max-width: 330px; width: 100%; text-align: right; box-shadow: 0 10px 30px rgba(0,0,0,0.25); }
    .alt-item { background: var(--bg); padding: 10px 12px; border-radius: 10px; margin-bottom: 8px; font-size: 13px; }
...
<body>
  ...
  <!-- חלון קופץ: חלופות ולוח זמנים לשעתיים הקרובות -->
  <div id="alt-modal" class="modal-backdrop">
    <div class="modal-box">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <h3 id="alt-modal-title" style="font-size:16px;">חלופות וזמנים לקו</h3>
      ...
      <div style="font-size:12.5px; color:var(--sec); margin-bottom:12px;">זמני יציאה קרובים (לשעתיים הבאות):</div>
      <div id="alt-departures-list" style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:14px;"></div>
      <div style="font-size:12.5px; color:var(--sec); margin-bottom:8px;">קווים חלופיים שמגיעים לאותו יעד:</div>
  ...
  <div class="card">
    ...
    <div class="input-row">
      ...
      <label>יעד</label>
      <input type="text" id="userDestInput" placeholder="לאן להגיע? (למשל: ניצנה)">
  ...
  <script>
    ...
    const exactCoords = {
      userOrigin: { lat: 32.7842, lon: 35.1718, name: 'עדי' },
      userDest: { lat: 30.8750, lon: 34.4280, name: 'ניצנה' },
      driverOrigin: { lat: 32.7842, lon: 35.1718, name: 'עדי' },
      driverDest: { lat: 31.7870, lon: 35.2010, name: 'ירושלים' }
    };
    ...
    // זיהוי שגיאת מיקום אמיתית בלבד (ספארי / כרום)
    function useCurrentGPS() {
      if (!navigator.geolocation) {
        alert('שירות מיקום אינו נתמך במכשיר זה');
        return;
      }
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        // התראה תקפוץ אך ורק אם המשתמש סירב מפורשות בהרשאה (PERMISSION_DENIED)
        if (err.code === 1) { // 1 = PERMISSION_DENIED
          const isChrome = /chrome|crios/i.test(navigator.userAgent);
          if (isChrome) {
            alert('הגישה למיקום חסומה ב-Chrome. כדי להפעיל: לחץ על סמל המנעול / ההגדרות ליד כתובת האתר -> הרשאות -> אפשר מיקום.');
          } else {
            alert('הגישה למיקום חסומה ב-Safari. כדי להפעיל: לחץ על סמל aA / הגדרות האתר משמאל לכתובת למטה -> הגדרות אתר -> מיקום -> אפשר.');
          }
        }
      }, { enableHighAccuracy: true, timeout: 8000, maximumAge: 60000 });
    }

    // פתיחת חלון חלופות ולוחות זמנים לשעתיים הקרובות
    function openAlternativesModal(lineName, rawMins, intervalMins, altsJson) {
      document.getElementById('alt-modal-title').innerText = 'חלופות וזמנים עבור ' + lineName;
      
      const depContainer = document.getElementById('alt-departures-list');
      depContainer.innerHTML = '';
      
      // יצירת זמנים לשעתיים הבאות (במרווחי יציאה)
      for (let i = 0; i <= 120; i += Number(intervalMins || 20)) {
        ...
        pill.innerText = (i === 0 ? 'נוכחי: ' : '+ ' + i + ' דק׳: ') + timeStr;
      ...
      if (alternatives.length === 0) {
        ...
        alternatives.forEach(alt => {
          ...
          if (typeof alt === 'object') {
            div.innerHTML = '<b>' + alt.name + '</b> (' + alt.frequency + ')<br><span style="color:var(--sec); font-size:12px;">' + alt.desc + '</span>';
          } else {
            div.innerHTML = '<b>' + alt + '</b> - מסלול מקביל';
          }
