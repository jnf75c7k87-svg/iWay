...
// רשת תחנות הרכבת המרכזיות בישראל לצורך חישוב תחנת הירידה הקרובה ביותר ליעד
const TRAIN_NETWORK_STATIONS = [
  { name: 'רכבת נתניה', lat: 32.3190, lon: 34.8660 },
  { name: 'רכבת בית יהושע', lat: 32.2610, lon: 34.8640 },
  { name: 'רכבת נתניה - ספיר', lat: 32.2820, lon: 34.8650 },
  { name: 'רכבת הרצליה', lat: 32.1629, lon: 34.8252 },
  { name: 'רכבת תל אביב אוניברסיטה', lat: 32.1032, lon: 34.8049 },
  { name: 'רכבת תל אביב סבידור מרכז', lat: 32.0835, lon: 34.7983 },
  { name: 'רכבת תל אביב השלום', lat: 32.0734, lon: 34.7925 },
  { name: 'רכבת תל אביב ההגנה', lat: 32.0538, lon: 34.7788 },
  { name: 'רכבת חדרה מערב', lat: 32.4410, lon: 34.9080 },
  { name: 'רכבת בנימינה', lat: 32.5140, lon: 34.9520 },
  { name: 'רכבת חיפה חוף הכרמל', lat: 32.7930, lon: 34.9570 },
  { name: 'רכבת חיפה מרכז השמונה', lat: 32.8220, lon: 34.9970 },
  ...
  { name: 'רכבת ראשון לציון משה דיין', lat: 31.9877, lon: 34.7570 },
  { name: 'רכבת רחובות', lat: 31.9080, lon: 34.8080 },
  { name: 'רכבת אשדוד עד הלום', lat: 31.7770, lon: 34.6660 },
  { name: 'רכבת אשקלון', lat: 31.6750, lon: 34.5950 },
  { name: 'רכבת קריית גת', lat: 31.6035, lon: 34.7738 },
  { name: 'רכבת באר שבע מרכז', lat: 31.2435, lon: 34.7972 }
];

// מציאת תחנת הרכבת הקרובה ביותר ליעד
...
// נקודות מעבר לאיסוף והורדה
const ALL_HUBS = [
  { id: 'h_tlv_hashalom', name: 'רכבת ת"א השלום', type: 'train', transitType: 'רכבת ישראל', lineName: 'רכבת בינעירונית (צפון/דרום)', platform: 'רציף 3', lat: 32.0734, lon: 34.7925 },
  { id: 'h_tlv_savidor', name: 'ת"א סבידור מרכז', type: 'train', transitType: 'רכבת ישראל', lineName: 'רכבת בינעירונית ישירה', platform: 'רציף 4', lat: 32.0835, lon: 34.7983 },
  { id: 'h_shapirim', name: 'חניון שפירים הנתיב המהיר', type: 'bus', transitType: 'שאטל ישיר', lineName: 'שאטל מהיר לתל אביב', platform: 'מתחם שאטלים', lat: 32.0078, lon: 34.8322 },
  { id: 'h_herzliya', name: 'רכבת הרצליה', type: 'train', transitType: 'רכבת ישראל', lineName: 'קו החוף והשרון', platform: 'רציף 2', lat: 32.1629, lon: 34.8252 },
  { id: 'h_motzkin', name: 'רכבת קריית מוצקין', type: 'train', transitType: 'רכבת ישראל', lineName: 'קו חוף וצפון', platform: 'רציף 1', lat: 32.8335, lon: 35.0805 },
  { id: 'h_mifratz', name: 'מרכזית המפרץ', type: 'train', transitType: 'רכבת ומטרונית', lineName: 'רכבת החוף + מטרונית', platform: 'רציף 3', lat: 32.7933, lon: 35.0344 },
  { id: 'h_hamovill', name: 'צומת המוביל / מחלף המוביל', type: 'bus', transitType: 'אוטובוס בינעירוני מהיר', lineName: 'קו 430 (סופרבוס)', platform: 'רציף 2', lat: 32.7562, lon: 35.2341 },
  { id: 'h_kgat', name: 'רכבת קריית גת', type: 'train', transitType: 'רכבת ישראל', lineName: 'רכבת באר שבע ⟵ הרצליה', platform: 'רציף 1', lat: 31.6035, lon: 34.7738 },
  { id: 'h_plugot', name: 'צומת פלוגות', type: 'bus', transitType: 'אוטובוס בינעירוני', lineName: 'קו 369 (מטרופולין)', platform: 'תחנה בינעירונית', lat: 31.6214, lon: 34.7478 },
  { id: 'h_b7', name: 'רכבת באר שבע מרכז', type: 'train', transitType: 'רכבת ישראל', lineName: 'רכבת מהירה למרכז', platform: 'רציף 2', lat: 31.2435, lon: 34.7972 },
  { id: 'h_ashkelon', name: 'רכבת אשקלון', type: 'train', transitType: 'רכבת ישראל', lineName: 'קו אשקלון ⟵ הרצליה', platform: 'רציף 2', lat: 31.6750, lon: 34.5950 }
];

// מנוע אופטימיזציה דינמי חכם
function runSmartOptimizer(body) {
  ...
  // 1. איתור אוטומטי של תחנת הירידה הקרובה ביותר ליעד
  ...
  for (const hub of ALL_HUBS) {
    ...
    const totalTripMinutes = driveWithDriver + trainRideMinutes + finalLegMinutes + 8;
    ...
    if (driveWithDriver > 0) {
      timeline.push({
        ...
        icon: '🚗',
        badge: routeType === 'driver_route' ? 'נסיעה עם הנהג' : 'הקפצה מהמוצא',
        title: `איסוף מנקודת המוצא`,
        desc: `נסיעה משותפת כ-${driveWithDriver} דקות עד לתחנת ${hub.name}`
      });
      timeline.push({
        ...
        desc: routeType === 'driver_route' ? `סטיית הנהג ממסלולו: +${detourMin} דקות בלבד` : `זמן הקפצה: ${driveWithDriver} דקות`
      });
    } else {
      timeline.push({
        time: formatMinutesToTime(tPickup),
    ...
    timeline.push({
      ...
      title: `עולים ב-${hub.name} (${hub.platform || 'רציף ראשי'})`,
      desc: `קו ישיר לכיוון ${optimalAlightStation.name} • כ-${trainRideMinutes} דקות נסיעה`
    });
    ...
    timeline.push({
      ...
      icon: '🛑',
      badge: 'ירידה',
      title: `יורדים בתחנה הקרובה ליעד: ${optimalAlightStation.name}`,
      desc: `התחנה האופטימלית ליעדך (מרחק של כ-${(distFinalKm).toFixed(1)} ק״מ מהיעד)`
    });
    ...
    if (hasDestPickup) {
      timeline.push({
        ...
        desc: `נסיעה ברכב של כ-${driveFinalLeg} דקות ישירות ליעד הסופי (ברדיוס של עד ${destRideRadius} דק׳)`
      });
    } else {
      timeline.push({
        time: formatMinutesToTime(tAlightTransit + 2),
        ...
        desc: `נסיעה בקו מקומי / הליכה כ-${finalLegMinutes} דקות עד לרחוב המבוקש`
      });
    }
    ...
    candidates.push({
      ...
      hasDestPickup,
      score,
      whyReason: `הורדה נוחה ב-${hub.name}, נסיעה ישירה וירידה ב-${optimalAlightStation.name} הסמוכה ליעד.`,
      timeline
    });
  }
...
app.get('/v1/geocode', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/9.0' } });
...
<body>
  ...
  <div id="driver-route-section">
    ...
    <div class="card">
      ...
      <input type="range" id="detourRange" min="5" max="120" step="5" value="10" oninput="document.getElementById('detourText').innerText = this.value + ' דקות'; triggerSearch();">
    </div>
  </div>
  ...
  <div class="card">
    <div class="slider-header">
      ...
      <span class="slider-val" id="destRideText">20 דקות ברכב</span>
    </div>
    <input type="range" id="destRideRange" min="0" max="60" step="5" value="20" oninput="document.getElementById('destRideText').innerText = this.value == 0 ? '0 דקות (בלי איסוף)' : 'עד ' + this.value + ' דק׳ ברכב'; triggerSearch();">
  </div>
  ...
  <script>
    ...
    const exactCoords = {
      ...
      driverDest: { lat: 32.0720, lon: 34.8100, name: 'גבעתיים' }
    };
    ...
    async function triggerSearch() {
      ...
      btnText.innerText = 'מחשב מסלול חכם...';
    ...
    function renderResults(results) {
      ...
      if (best.hub && best.hub.lat) {
        updateMarkerOnMap('hub', best.hub.lat, best.hub.lon, 'תחנת עלייה: ' + best.hub.name, '#FF3B30');
      }
      ...
      results.forEach((r, idx) => {
        ...
        card.innerHTML = \`
          ...
          <div style="font-size: 15px; font-weight: 600; margin-top: 6px;">הורדה ב-\${r.hub.name} ⟵ ירידה ב-\${r.optimalAlightStation}</div>
