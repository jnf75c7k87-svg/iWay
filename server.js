...
const REGIONAL_BUS_DATABASE = [
  ...
  { tags: ['פלוגות', 'תל אביב', 'מרכז'], lineNum: '369', operator: 'מטרופולין', title: 'קו 369: באר שבע ⟵ פלוגות ⟵ ת"א סבידור', intervalMins: 30, terminalA: 'באר שבע', terminalB: 'תל אביב סבידור מרכז', stopId: '13524', platform: 'מסלול צפון', alts: ['קו 370', 'קו 348'] },
  { tags: ['המוביל', 'טבריה', 'עמקים'], lineNum: '430', operator: 'אגד', title: 'קו 430: חיפה ⟵ מחלף המוביל ⟵ טבריה', intervalMins: 45, terminalA: 'חיפה מרכזית המפרץ', terminalB: 'טבריה מרכזית', stopId: '54210', platform: 'רציף 2', alts: ['קו 434', 'קו 500'] },
  { tags: ['באר שבע', 'ניצנה', 'עזוז'], lineNum: '44', operator: 'דן בדרום', title: 'קו 44: באר שבע ⟵ מסוף ניצנה', intervalMins: 60, terminalA: 'באר שבע מרכזית', terminalB: 'מסוף ניצנה', stopId: '39006', platform: 'רציף 8', alts: ['קו 45'] },
  { tags: ['נתניה', 'פרדסיה'], lineNum: '26', operator: 'קווים', title: 'קו 26: רכבת נתניה ⟵ פרדסיה', intervalMins: 25, terminalA: 'רכבת נתניה', terminalB: 'פרדסיה', stopId: '37302', platform: 'תחנה מזרחית', alts: ['קו 39', 'קו 47'] },
  { tags: ['תל אביב', 'סוקולוב', 'פנקס'], lineNum: '5', operator: 'דן', title: 'קו 5: רכבת ת"א סבידור ⟵ רחוב סוקולוב', intervalMins: 10, terminalA: 'ת"א סבידור מרכז', terminalB: 'צפון תל אביב', stopId: '36004', platform: 'גשר מודעי', alts: ['קו 25', 'קו 125'] }
];
...
function findMatchingBus(fromText, toText) {
  ...
  for (const b of REGIONAL_BUS_DATABASE) {
    if (b.tags.some(tag => f.includes(tag.toLowerCase())) && b.tags.some(tag => t.includes(tag.toLowerCase()))) {
      return b;
    }
  }
  for (const b of REGIONAL_BUS_DATABASE) {
...
function runMasterTransitEngine(body) {
  ...
  if (totalDistanceKm <= 9.0) {
    ...
    return [{
      ...
      whyReason: `נסיעה מקומית קצרה (${totalDistanceKm.toFixed(1)} ק״מ). נסיעה ישירה ברכב או קו עירוני.`,
      timeline: [
        {
          ...
          desc: `זמן נסיעה מוערך: כ-${driveDirectMins} דקות.`,
          lat: userOrigin.lat,
          lon: userOrigin.lon
        },
        {
...
<body>
  ...
  <div id="alt-modal" class="modal-backdrop">
    <div class="modal-box">
      ...
      <div style="font-size:12.5px; color:var(--sec); margin-bottom:8px;">זמני יציאה קרובים (שעתיים קדימה):</div>
      <div id="alt-departures-list" style="display:flex; flex-wrap:wrap; gap:6px; margin-bottom:14px;"></div>
      <div style="font-size:12.5px; color:var(--sec); margin-bottom:8px;">קווים חלופיים (לחץ לצפייה בלוח זמנים):</div>
  ...
  <div id="map-modal" class="map-modal">
    ...
    <div class="map-modal-search">
      <input type="text" id="modalSearchInput" placeholder="הקלד עיר, צומת או רחוב..." autocomplete="off" oninput="handleModalSearchInput()" onkeydown="if(event.key==='Enter'){event.preventDefault();executeModalSearch();}">
  ...
  <div class="card" style="overflow:visible;">
    <div class="input-row">
      ...
      <label>מוצא</label>
      <input type="text" id="userOriginInput" placeholder="היכן אתה נמצא? (הקלד עיר או כתובת)" autocomplete="off" oninput="handleInputAutocomplete('userOrigin')">
    ...
    <div class="input-row">
      ...
      <label>יעד</label>
      <input type="text" id="userDestInput" placeholder="לאן להגיע? (למשל: מעלה אדומים)" autocomplete="off" oninput="handleInputAutocomplete('userDest')">
  ...
  <script>
    ...
    // מאגר יישובים מובנה בישראל לפעולה מיידית וללא תקלות רשת
    const BUILTIN_ISRAEL_LOCATIONS = [
      { name: 'מעלה אדומים', fullName: 'מעלה אדומים, מחוז יהודה ושומרון', lat: 31.7915, lon: 35.2975 },
      { name: 'ירושלים', fullName: 'ירושלים, ישראל', lat: 31.7878, lon: 35.2015 },
      { name: 'תל אביב', fullName: 'תל אביב-יפו, ישראל', lat: 32.0835, lon: 34.7983 },
      { name: 'באר שבע', fullName: 'באר שבע, ישראל', lat: 31.2435, lon: 34.7972 },
      { name: 'חיפה', fullName: 'חיפה, ישראל', lat: 32.7933, lon: 35.0344 },
      { name: 'נתניה', fullName: 'נתניה, ישראל', lat: 32.3190, lon: 34.8660 },
      { name: 'פרדסיה', fullName: 'פרדסיה, השרון', lat: 32.3040, lon: 34.9120 },
      { name: 'אשקלון', fullName: 'אשקלון, ישראל', lat: 31.6750, lon: 34.5950 },
      { name: 'קריית גת', fullName: 'קריית גת, ישראל', lat: 31.6035, lon: 34.7738 },
      { name: 'צומת פלוגות', fullName: 'צומת פלוגות, כביש 40 / 35', lat: 31.6214, lon: 34.7478 },
      { name: 'עדי', fullName: 'עדי, גליל תחתון', lat: 32.7842, lon: 35.1718 },
      { name: 'מחלף המוביל', fullName: 'מחלף המוביל, כביש 77', lat: 32.7562, lon: 35.2341 },
      { name: 'קריית מוצקין', fullName: 'קריית מוצקין, הקריות', lat: 32.8335, lon: 35.0805 },
      { name: 'שדרות', fullName: 'שדרות, עוטף עזה', lat: 31.5270, lon: 34.5970 },
      { name: 'ניצנה', fullName: 'מסוף ניצנה / קהילת ניצנה', lat: 30.8750, lon: 34.4280 },
      { name: 'אריאל', fullName: 'אריאל, שומרון', lat: 32.1060, lon: 35.1840 },
      { name: 'אילת', fullName: 'אילת, ישראל', lat: 29.5581, lon: 34.9482 },
      { name: 'טבריה', fullName: 'טבריה, ישראל', lat: 32.7940, lon: 35.5312 },
      { name: 'כרמיאל', fullName: 'כרמיאל, הגליל', lat: 32.9190, lon: 35.3090 },
      { name: 'עפולה', fullName: 'עפולה, עמק יזרעאל', lat: 32.6140, lon: 35.2950 },
      { name: 'בית שאן', fullName: 'בית שאן, ישראל', lat: 32.5080, lon: 35.5010 },
      { name: 'הרצליה', fullName: 'הרצליה, ישראל', lat: 32.1629, lon: 34.8252 },
      { name: 'רחוב נחום סוקולוב, תל אביב', fullName: 'נחום סוקולוב, תל אביב', lat: 32.0898, lon: 34.7788 }
    ];
    ...
    // חיפוש כתובות אמין וישיר ללא תלות בחסימת שרת
    async function universalGeocode(queryText) {
      const q = queryText.trim().replace(/פרדסייה/g, 'פרדסיה').toLowerCase();
      if (!q || q.length < 2) return [];

      // 1. קודם בדיקה במאגר המקומי המהיר
      const localMatches = BUILTIN_ISRAEL_LOCATIONS.filter(item => 
        item.name.toLowerCase().includes(q) || item.fullName.toLowerCase().includes(q) || q.includes(item.name.toLowerCase())
      );

      // 2. פנייה מרובת מנועים ישירות מהדפדפן
      try {
        const url = 'https://nominatim.openstreetmap.org/search?format=json&countrycodes=il&accept-language=he&limit=6&q=' + encodeURIComponent(q);
        const resp = await fetch(url);
        const list = await resp.json();
        const netMatches = (list || []).map(it => ({
    ...
    function useCurrentGPS() {
      ...
      if (!navigator.geolocation) {
        status.innerText = 'שירות מיקום אינו נתמך בדפדפן זה';
        return;
      }
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        err => {
          status.innerText = 'לא ניתן לאתר מיקום אוטומטית. נא להקליד ידנית את העיר או הכתובת.';
        },
    ...
    async function triggerSearch() {
      ...
      // תרגום מוצא אוטומטי אם נכתב טקסט
      ...
      // תרגום יעד אוטומטי אם נכתב טקסט
