...
function runSmartOptimizer(body) {
  const {
    ...
    originRideMin = 0,
    destRideMin = 0,
    maxWalkOriginMin = 20, // סליידר מקסימום הליכה מהמוצא לתחנה
    mode = 'smart'
  } = body;
  ...
  for (const hub of ALL_HUBS) {
    ...
    if (routeType === 'driver_route') {
      ...
      if (rideFromOriginTime > 0) {
        ...
        // ללא הקפצה ברכב (0 דק'): סינון לפי סליידר ההליכה שהמשתמש אישר
    ...
    if (hasDestPickup) {
      ...
      timeline.push({
        ...
        desc: `קו מקומי או הליכה רגלית כ-${finalLegMinutes} דקות`,
        lat: optimalAlightStation.lat,
        lon: optimalAlightStation.lon
      });
    }
    ...
    if (mode === 'easy') score = totalTripMinutes + (hasDestPickup ? -20 : 0) + (walkToHubMin > 15 ? 10 : 0);
    ...
    candidates.push({
      ...
      hasDestPickup,
      score,
      whyReason: driveWithDriver > 0
        ? `מעבר ברכב דרך ${hub.name} (${hub.lineBadge}) וירידה ב-${optimalAlightStation.name}.`
        : `הליכה של ${walkToHubMin} דק׳ ל-${hub.name}, נסיעה ישירה וירידה ב-${optimalAlightStation.name}.`,
      timeline
    });
  }
...
app.get('/v1/geocode', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/14.0' } });
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Text", sans-serif; -webkit-tap-highlight-color: transparent; }
    ...
    .perm-modal { position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px); z-index: 20000; display: flex; align-items: center; justify-content: center; padding: 20px; }
...
<body>
  ...
  <div id="rides-section" style="display: none;">
    ...
    <div class="card">
      ...
      <!-- סליידר מקסימום הליכה לתחנת המוצא - מופיע כשללא הקפצה -->
  ...
  <script>
    ...
    function useCurrentGPS() {
      ...
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        alert('ספארי חסם את הגישה למיקום. שחרר את החסימה: לחץ על סמל aA או הגדרות האתר משמאל לשורת הכתובת בספארי -> הגדרות אתר -> מיקום -> אפשר.');
    ...
    function renderResults(results) {
      ...
      if (!results || results.length === 0) {
        container.innerHTML = '<div class="card">לא נמצאו תחנות בטווח ההליכה שהוגדר. נסה להעלות את סליידר ההליכה לתחנה הראשונה, או להגדיר זמן הקפצה ברכב.</div>';
        return;
      }
