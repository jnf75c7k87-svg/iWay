...
// קווי אוטובוס עירוניים ואזוריים אמיתיים לפי קואורדינטות וערים
const REAL_BUS_LINES = [
  ...
  {
    corridor: 'tlv_south_center',
    lineNum: '25',
    operator: 'דן',
    title: 'קו 25 (דן): בת ים ⟵ יפו ⟵ מרכז תל אביב ⟵ אוניברסיטה',
    originTerminal: 'בת ים',
    destTerminal: 'אוניברסיטת תל אביב',
    departureSchedule: [5, 17, 29, 41, 53],
    intervalMins: 12,
    boardStation: 'אלנבי / רוטשילד',
    stopId: '20500',
    alightStation: 'אבן גבירול / פנקס',
    alightStopId: '21300',
    alts: ['קו 125 (דן)', 'קו 189 (דן)']
  },
  // ירושלים
  {
    ...
    destTerminal: 'מעלה אדומים (שדרות החושן)',
...
const ISRAEL_TRAIN_NETWORK = [
  {
    ...
    stations: [
      ...
      { name: 'רכבת באר שבע צפון', stopId: '39005', platform: 'רציף 2', lat: 31.2610, lon: 34.8110 },
...
function runMasterTransitEngine(body) {
  ...
  // מרחק קצר (עד 7 ק"מ): מציאת קו אוטובוס אמיתי מוגדר ולא ניסוח כללי
    ...
    return [{
      ...
      timeline: [
        ...
        },
        {
          ...
          alternatives: bus.alts.map(a => ({ name: a, desc: 'קו מקביל לאותו יעד' })),
...
<body>
  ...
  <div id="map-modal" class="map-modal">
    ...
    <div class="map-modal-search">
      <input type="text" id="modalSearchInput" placeholder="הקלד רחוב, עיר או צומת..." autocomplete="off" oninput="handleModalSearchInput()" onkeydown="if(event.key==='Enter'){event.preventDefault();executeModalSearch();}">
  ...
  <div class="card" style="overflow:visible;">
    <div class="input-row">
      ...
      <label>מוצא</label>
      <input type="text" id="userOriginInput" placeholder="היכן אתה נמצא? (הקלד רחוב או עיר)" autocomplete="off" oninput="handleInputAutocomplete('userOrigin')">
    ...
    <div class="input-row">
      ...
      <label>יעד</label>
      <input type="text" id="userDestInput" placeholder="לאן להגיע? (למשל: סוקולוב תל אביב)" autocomplete="off" oninput="handleInputAutocomplete('userDest')">
  ...
  <script>
    ...
    // מנוע חיפוש רחובות, ערים וצמתים
    async function universalGeocode(queryText) {
      const q = queryText.trim().replace(/פרדסייה/g, 'פרדסיה');
    ...
    // כפתור מיקום נוכחי מהיר ואמין במובייל
    function useCurrentGPS() {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        err => {
          // ניסיון fallback מבוסס IP אם ה-GPS מושהה
    ...
    async function triggerSearch() {
      ...
      // בדיקה אם הוזנו קואורדינטות ישירות
      function parseCoords(text) {
        const parts = text.split(',');
        if (parts.length === 2) {
          const lat = parseFloat(parts[0].trim());
          const lon = parseFloat(parts[1].trim());
          if (!isNaN(lat) && !isNaN(lon)) return { lat, lon, name: text };
        }
        return null;
      }

      if (!exactCoords.userOrigin || exactCoords.userOrigin.name !== uOriginVal) {
        const direct = parseCoords(uOriginVal);
        if (direct) {
      ...
      if (!exactCoords.userDest || exactCoords.userDest.name !== uDestVal) {
        const direct = parseCoords(uDestVal);
        if (direct) {
      ...
      if (dOriginVal && (!exactCoords.driverOrigin || exactCoords.driverOrigin.name !== dOriginVal)) {
        const direct = parseCoords(dOriginVal);
      ...
      if (dDestVal && (!exactCoords.driverDest || exactCoords.driverDest.name !== dDestVal)) {
        const direct = parseCoords(dDestVal);
    ...
    function renderResults(results) {
      ...
      if (!results || results.length === 0) {
        container.innerHTML = '<div class="card">לא נמצאו נקודות מעבר מתאימות. נסה להעלות את טווח הזמנים.</div>';
        return;
      }
