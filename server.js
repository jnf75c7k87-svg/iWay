...
function runSmartOptimizer(body) {
  ...
  for (const hub of ALL_HUBS) {
    ...
    if (routeType === 'driver_route') {
      ...
      if (originRideMin > 0) {
        if (driveFromOrigin <= originRideMin) {
          isEligible = true;
          driveWithDriver = driveFromOrigin;
          detourMin = driveFromOrigin;
        }
      } else {
        // ללא הקפצה ברכב (0 דק'): רק תחנות במרחק הליכה סביר (עד 3.5 ק"מ)
...
app.get('/v1/geocode', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/11.0' } });
  ...
  <script>
    ...
    function useCurrentGPS() {
      ...
      navigator.geolocation.getCurrentPosition(pos => {
        ...
        alert('נא לאשר גישה למיקום בהגדרות הדפדפן כדי להשתמש במיקום נוכחי');
    ...
    function renderResults(results) {
      ...
      if (!results || results.length === 0) {
        container.innerHTML = '<div class="card">לא נמצאו תחנות מתאימות בטווח שנבחר. אם בחרת ללא הקפצה, ודא שיש תחנה במרחק הליכה, או הגדר הקפצה בסליידר.</div>';
        return;
      }
