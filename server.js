...
// תחנות רכבת ארציות עם מק"ט תחנה ורציפים מדויקים
const TRAIN_NETWORK_STATIONS = [
  ...
  { name: 'רכבת חיפה מרכז השמונה', stopId: '34004', platform: 'רציף 2', trainNum: 'רכבת 218', lat: 32.8220, lon: 34.9970 },
...
// תחנות מעבר מוגדרות עם מק"ט תחנה רשמי, מספרי קווים ורציפים
const ALL_HUBS = [
  { 
    id: 'h_plugot', 
    name: 'צומת פלוגות (תחנה בינעירונית)', 
    type: 'bus', 
    transitType: 'אוטובוס בינעירוני', 
    lineBadge: 'קו 369', 
    lineName: 'קו 369 (מטרופולין) לכיוון ת"א', 
    stopId: '13524', 
    platform: 'מסלול צפון', 
    lat: 31.6214, 
    lon: 34.7478 
  ...
  { 
    ...
    lineBadge: 'רכבת 142', 
    lineName: 'רכבת בינעירונית (באר שבע ⟵ נהריה)', 
    stopId: '36006', 
    platform: 'רציף 3', 
    lat: 32.0734, 
    lon: 34.7925 
  ...
  { 
    ...
    transitType: 'רכבת ישראל', 
    lineBadge: 'רכבת 218', 
    lineName: 'רכבת מהירה לצפון (ת"א ⟵ חיפה)', 
    stopId: '36004', 
    platform: 'רציף 4', 
    lat: 32.0835, 
    lon: 34.7983 
  ...
  { 
    ...
    lineBadge: 'שאטל 1', 
    lineName: 'שאטל ישיר לקריה / עזריאלי', 
    stopId: '10022', 
    platform: 'מתחם שאטלים', 
    lat: 32.0078, 
    lon: 34.8322 
  },
  { 
  ...
  { 
    id: 'h_hamovill', 
    name: 'מחלף המוביל (מסוף בינעירוני)', 
    type: 'bus', 
    transitType: 'אוטובוס בינעירוני מהיר', 
    lineBadge: 'קו 430', 
    lineName: 'קו 430 (סופרבוס) לצפון ולמרכז', 
    stopId: '54210', 
    platform: 'רציף 2', 
    lat: 32.7562, 
    lon: 35.2341 
  ...
  { 
    ...
    transitType: 'רכבת ישראל', 
    lineBadge: 'רכבת 142', 
    lineName: 'רכבת מהירה למרכז (באר שבע ⟵ הרצליה)', 
    stopId: '39002', 
    platform: 'רציף 1', 
    lat: 31.6035, 
    lon: 34.7738 
  ...
  for (const hub of ALL_HUBS) {
    ...
    if (driveWithDriver > 0) {
      ...
      timeline.push({
        ...
        desc: `סטיית נהג ממסלולו: +${detourMin} דק׳ • ${hub.platform}`,
        lat: hub.lat,
        lon: hub.lon
      });
    } else {
    ...
    timeline.push({
      ...
      desc: `תחנה #${hub.stopId} (${hub.platform}) • ירידה מתוכננת ב-${optimalAlightStation.name} (כ-${trainRideMinutes} דק׳)`,
      lat: hub.lat,
      lon: hub.lon
    });
    ...
    timeline.push({
      ...
      desc: `${optimalAlightStation.platform} • מרחק מהיעד הסופי: כ-${(distFinalKm).toFixed(1)} ק״מ`,
    ...
    if (hasDestPickup) {
      ...
      timeline.push({
        ...
        desc: `קו מקומי או הליכה רגלית כ-${finalLegMinutes} דקות עד לכתובת היעד`,
        lat: optimalAlightStation.lat,
        lon: optimalAlightStation.lon
      });
    }
    ...
    candidates.push({
      ...
      hasDestPickup,
      score,
      whyReason: `הורדה ב-${hub.name} (${hub.lineBadge}) וירידה ב-${optimalAlightStation.name} הקרובה ביותר ליעד.`,
      timeline
    });
  }
...
app.get('/v1/geocode', async (req, res) => {
  ...
  try {
    ...
    const resp = await fetch(url, { headers: { 'User-Agent': 'iWayApp-Israel/10.0' } });
    ...
    function renderResults(results) {
      ...
      results.forEach((r, idx) => {
        ...
        card.innerHTML = \`
          ...
          <div style="font-size: 15px; font-weight: 600; margin-top: 6px;">תחנת מעבר: \${r.hub.name} ⟵ ירידה ב-\${r.optimalAlightStation}</div>
          <div class="result-sub">\${r.whyReason}</div>

          <!-- פירוט שלבי המסלול כולל כפתור ניווט בכל שלב -->
