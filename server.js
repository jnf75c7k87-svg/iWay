...
const REGIONAL_BUS_DATABASE = [
  { tags: ['ירושלים', 'מעלה אדומים'], lineNum: '174', operator: 'אלקטרה אפיקים', title: 'קו 174: ירושלים ⟵ מעלה אדומים', intervalMins: 15, terminalA: 'ירושלים', terminalB: 'מעלה אדומים', stopId: '30050', platform: 'תחנה 4', alts: ['קו 176', 'קו 177'] },
  ...
  { tags: ['תל אביב', 'סוקולוב', 'דיזנגוף', 'פנקס'], lineNum: '5', operator: 'דן', title: 'קו 5: רכבת ת"א סבידור ⟵ רחוב סוקולוב / דיזנגוף', intervalMins: 10, terminalA: 'ת"א סבידור מרכז', terminalB: 'צפון תל אביב', stopId: '36004', platform: 'גשר מודעי', alts: ['קו 25', 'קו 125'] },
  { tags: ['אריאל', 'תל אביב'], lineNum: '286', operator: 'תנופה', title: 'קו 286: אריאל ⟵ ת"א סבידור', intervalMins: 20, terminalA: 'אריאל', terminalB: 'ת"א סבידור מרכז', stopId: '19001', platform: 'מסוף אריאל', alts: ['קו 186'] },
  { tags: ['באר שבע', 'אילת'], lineNum: '390', operator: 'אגד', title: 'קו 390: באר שבע ⟵ אילת', intervalMins: 60, terminalA: 'באר שבע מרכזית', terminalB: 'אילת מרכזית', stopId: '39010', platform: 'רציף בינעירוני', alts: ['קו 394', 'קו 397'] }
];
...
<body>
  ...
  <script>
    ...
    // מנוע חיפוש רחובות, ערים וצמתים ישיר מרובה-מקורות (Photon + OSM + Nominatim)
    async function universalGeocode(queryText) {
      ...
      // מקור 1: Photon API (מנוע רחובות ייעודי שתומך במספרי בתים, רחובות ושכונות בישראל)
      try {
      ...
      // מקור 2: Nominatim ישיר כגיבוי לשכונות, צמתים ומחלפים
      if (results.length < 4) {
        try {
    ...
    // Reverse Geocoding חכם של מיקום GPS לשם רחוב בעברית
    async function reverseGeocodeCoords(lat, lon) {
      try {
        const revUrl = 'https://nominatim.openstreetmap.org/reverse?format=json&accept-language=he&lat=' + lat + '&lon=' + lon;
      ...
      return 'מיקום נוכחי (' + lat.toFixed(4) + ', ' + lon.toFixed(4) + ')';
    }

    // מנגנון מיקום נוכחי מדויק ועמיד למובייל ולמחשב
    function useCurrentGPS() {
      const status = document.getElementById('gps-status-msg');
      status.style.display = 'block';
      status.innerText = 'מאתר מיקום GPS...';
      ...
      navigator.geolocation.getCurrentPosition(
        async pos => {
          ...
          status.innerText = 'מזהה כתובת...';
          const placeName = await reverseGeocodeCoords(lat, lon);
          
          exactCoords.userOrigin = { lat, lon, name: placeName };
          document.getElementById('userOriginInput').value = placeName;
          status.style.display = 'none';
          
          updateMarkerOnMap('userOrigin', lat, lon, placeName, '#007AFF');
          map.setView([lat, lon], 15);
        },
        err => {
          status.innerText = 'לא ניתן לקבל מיקום. ודא שהרשאת המיקום מופעלת בהגדרות הדפדפן.';
        },
    ...
    async function triggerSearch() {
      ...
      if (!exactCoords.userOrigin || exactCoords.userOrigin.name !== uOriginVal) {
        ...
        if (list && list.length > 0) {
          ...
          alert('לא נמצא מיקום עבור מוצא: "' + uOriginVal + '". נסה לבחור מהרשימה הנפתחת או במפה.');
          return;
        }
      }
      ...
      if (!exactCoords.userDest || exactCoords.userDest.name !== uDestVal) {
        ...
        if (list && list.length > 0) {
          ...
          spinner.style.display = 'none';
          btnText.innerText = 'מצא לי דרך';
          alert('לא נמצא מיקום עבור יעד: "' + uDestVal + '". נסה לבחור מהרשימה הנפתחת או במפה.');
          return;
        }
      }
